---
title: "SCFA Analysis: Abbott Carbohydrate Obesity Project (Improved Version)"
_folding: show
    theme: united
  pdf_document:
    toc: true
...
**Statistical Method:** Box plots with individual data points overlaid. Statistical comparisons performed using Wilcoxon rank-sum tests between groups, with significance denoted by symbols (ns: p > 0.05, *: p ≤ 0.05, **: p ≤ 0.01, ***: p ≤ 0.001).
...
plot_by_group_carb <- sample_data_long %>%
  ...
  labs(title = "SCFA Concentrations by Group and Carbohydrate Type",
       x = "Group", y = "Concentration (mM)", fill = "Group") +
  pub_theme +
...
**Statistical Method:** Line plots showing group means ± standard error of the mean (SEM) across time points, stratified by carbohydrate type. Error bars represent 95% confidence intervals around the mean.
...
plot_time_series <- sample_data_long %>%
  ...
  labs(title = "SCFA Concentrations Over Time by Carbohydrate Type",
       x = "Time (hours)", y = "Mean Concentration ± SEM (mM)", color = "Carbohydrate Type") +
  pub_theme +
...
plot_delta_interaction <- delta_data %>%
  ...
  labs(title = "SCFA Response Magnitude: Group × Carbohydrate Interaction",
       ...
       x = "Carbohydrate Type", y = "Delta Concentration (48h - 0h) mM", fill = "Group") +
  pub_theme +
...
plot_delta_control_only <- delta_data %>%
  ...
  labs(
    ...
    x = "Carbohydrate Type", 
    y = "Change in Concentration (48h - 0h) mM"
  ) +
  pub_theme +
  theme(
...
plot_ratios_interaction <- ratio_data_long %>%
  mutate(ratio_type_label = str_replace_all(ratio_type, "_", " ") %>% str_to_title()) %>%
...
if(nrow(group_comparisons) > 0) {
  ...
  group_table <- group_comparisons %>%
    dplyr::select(analyte, contrast, estimate, SE, t.ratio, p.value, significance) %>%
    knitr::kable(digits = 4, caption = "Group Differences in SCFA Concentrations") %>%
...
if(nrow(time_comparisons) > 0) {
  ...
  time_table <- time_comparisons %>%
    dplyr::select(analyte, contrast, estimate, SE, t.ratio, p.value, significance) %>%
    knitr::kable(digits = 4, caption = "Temporal Changes in SCFA Concentrations") %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
  cat(time_table)
  cat("\n")
}
...
if(nrow(group_within_carb) > 0) {
  ...
  if(nrow(significant_group_within_carb) > 0) {
    group_carb_table <- significant_group_within_carb %>%
...
if(nrow(carb_within_group) > 0) {
  ...
  if(nrow(significant_carb_within_group) > 0) {
    carb_group_table <- significant_carb_within_group %>%
...
plot_trajectories <- sample_data_long %>%
  ...
  labs(title = "Individual Subject SCFA Trajectories by Carbohydrate Source",
       ...
       x = "Time (hours)", y = "Concentration (mM)", color = "Group") +
  pub_theme +
  theme(legend.position = "bottom")
